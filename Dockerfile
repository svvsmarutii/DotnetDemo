...
RUN if [ "$SONAR" = true ] ; then \
    ...
    && sonarAnalysisUrl="$(grep dashboardUrl .sonarqube/out/report-task.txt)" && export sonarAnalysisUrl=${sonarAnalysisUrl} || export sonarAnalysisUrl="$(echo "sonarscanner failed. Please check jenkins log ${env.JOB_URL} for exact reason")" \
